...
Import the following files (saved in your `data` subdirectory) into R using functions in `readr` package:
...
**Double** vectors somtimes known as **numeric** vectors. Number in R **default** as **double**. when you see a number like `1` in R, which you might think of as integer. The type actually as double. 
...
We can create a fator using function `factor`:
...
`tibble` is also a **data frame**, it tweak some older behaviours of `data.frame`, make it **easier** to analyze large datasets containing complex objects.
...
You can convert a `data.frame` objetc into `tibble` using function `as_tibble()`:
...
You can aslo convert a `tibble` into `data.frame` using function `as.data.frame()`:
